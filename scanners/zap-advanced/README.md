...
optional arguments:
  ...
  -r {XML,JSON,HTML,MD}, --report-type {XML,JSON,HTML,MD}
                        The OWASP ZAP Report Type u.
```

## Zap Configurations
The follwoing examples gives you an overview about all the different configuration options you have to configure the ZAP Advanced scanType. Please have a look into our `examples`. We provide a list of working examples to scan our `demo-apps` with the `zap-advanced-scan`.
  ...
  contexts:
    ...
    - name: scbcontext
      ...
      technology:
        ...
        excluded:
         - SCM
      # -- Authentiation Configuration that can be uses by ZAP Spider and/or Scanner. You need to reference the `context` name in the corresponding `zapConfiguration.spiders[0].context` and `zapConfiguration.scanners[0].context` section if you want to use them.
      authentication:
