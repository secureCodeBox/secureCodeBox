...
class ZapConfigureSpider():
    ...
    def wait_until_finished(self, spider_id: int):
        ...
        if(spider_id >= 0):
            while (int(self.__zap.spider.status(spider_id)) < 100):
                logging.debug("Spider(%s) progress %: %s", spider_id, self.__zap.spider.status(spider_id))
                time.sleep(1)
                
                logging.debug("Spider(%s) completed", spider_id)
            ...
            if num_urls == 0:
                ...
                logging.info("Spider(%s) found total: %s URLs", spider_id, str(num_urls))
    ...
    def __configure_spider(self, zap_spider, spider_config: collections.OrderedDict):
        ...
        if "maxDuration" in spider_config and spider_config['maxDuration'] >= 0:
        ...
        if "parseRobotsTxt" in spider_config:
            self.__check_zap_spider_result(
        ...
        if "parseSVNEntries" in spider_config:
            self.__check_zap_spider_result(
        ...
        if "threadCount" in spider_config and spider_config['threadCount'] >= 0:
            self.__check_zap_spider_result(
                spiderId=zap_spider.set_option_thread_count(str(spider_config['threadCount'])), 
                method="set_option_thread_count"
            )
        if "userAgent" in spider_config and len(spider_config['userAgent']) > 0:
            self.__check_zap_spider_result(
    ...
    def __check_zap_spider_result(self, spiderId: str, method: str):
        ...
        if "OK" != spiderId:
            logging.warn("Failed to configure '%s', result is: '%s'", method, spiderId)
