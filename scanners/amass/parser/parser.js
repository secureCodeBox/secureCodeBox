...
async function parse(fileContent) {
  ...
  return new Promise((resolve, reject) => {
    ...
    WITH relation_chain AS (
      ...
      JOIN relations r1 ON fqdn.id = r1.from_asset_id AND (r1.type = 'a_record' OR r1.type = 'aaaa_record')
      JOIN assets ips ON r1.to_asset_id = ips.id
    
      JOIN relations r2 ON ips.id = r2.to_asset_id AND r2.type = 'contains'
      JOIN assets cidr ON r2.from_asset_id = cidr.id
    
      JOIN relations r3 ON cidr.id = r3.to_asset_id AND r3.type = 'announces'
      JOIN assets asn ON r3.from_asset_id = asn.id
      ...
      JOIN relations r ON rc.asn_id = r.from_asset_id AND r.type = 'managed_by'
      JOIN assets a ON r.to_asset_id = a.id;`;
