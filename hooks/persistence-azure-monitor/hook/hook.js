...
async function handle({
  ...
  const processedFindings = JSON.stringify(findings.map(value => ({
    ...value,
  ...
  const headersToAuthenticate = 'POST\n' + contentLength + '\napplication/json\nx-ms-date:' + processingDate + '\n/api/logs';
  // Calculate the HMAC authenticator header
  const signature = crypto.createHmac('sha256', Buffer.from(sharedKey, 'base64')).update(headersToAuthenticate, 'utf-8').digest('base64');
  ...
  // Generate API URL
  const apiUrl = 'https://' + workspaceId + '.ods.opinsights.azure.com/api/logs?api-version=' + monitorApiVersion;

  // Send the request, handle the response
  fetch(apiUrl, {method: "POST", body: processedFindings, headers}).then(response => {
    if (response.status == 200) {
      console.log(`Data successfully sent to Azure Monitor.`);
      return;
    }
    const { Error: errorCode, Message: errorMsg } = response.json();
    console.error(`An error occured. Status Code: ${response.status}, status text: ${response.statusText}, Error: ${errorCode}, ErrorMsg: ${errorMsg}`);
  });
}
